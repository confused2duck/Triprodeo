...
export const createHost = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { name, email, password, phone, avatar, package: pkg, status } = req.body;
    ...
    const host = await import('../services/hosts.service').then((s) =>
      s.createHostAccount({ name, email, password, phone, avatar, package: pkg, status })
    );
    sendSuccess(res, host, 201, 'Host created');
  } catch (err) { next(err); }
};
...
export const updateHostProfile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { name, phone, avatar } = req.body;
    const host = await import('../services/hosts.service').then((s) =>
      s.updateHostProfile(req.params.hostId, { name, phone, avatar })
    );
    sendSuccess(res, host);
  } catch (err) { next(err); }
};

export const updateHostAccount = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const host = await import('../services/hosts.service').then((s) =>
      s.updateHostAccount(req.params.hostId, req.body)
    );
    sendSuccess(res, host);
  } catch (err) { next(err); }
};

export const deleteHostAccount = async (req: Request, res: Response, next: NextFunction) => {
  try {
    await import('../services/hosts.service').then((s) => s.deleteHostAccount(req.params.hostId));
    sendSuccess(res, null, 200, 'Host deleted');
  } catch (err) { next(err); }
};
