...
import { HostPackage, HostStatus } from '@prisma/client';
...
export const createHostAccount = async (data: {
  name: string;
  email: string;
  password: string;
  phone?: string;
  ...
  return prisma.host.create({
    data: {
      ...
      phone: data.phone,
      avatar: data.avatar,
      package: (data.package?.toUpperCase() as HostPackage) ?? 'BASIC',
      status: (data.status?.toUpperCase() as HostStatus) ?? 'ACTIVE',
    },
    select: {
      id: true,
      name: true,
...
export const updateHostAccount = async (
  hostId: string,
  data: {
    name?: string;
    email?: string;
  ...
  if (data.package) payload.package = data.package.toUpperCase();
...
export const deleteHostAccount = async (hostId: string) => {
  return prisma.host.delete({ where: { id: hostId } });
};
