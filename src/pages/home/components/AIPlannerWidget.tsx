...
export default function AIPlannerWidget() {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      setGenerating(false);
      const budgetLabel = budgets.find((b) => b.value === budget)?.label ?? `under ${budget}`;
      const prompt = `Plan a ${vibe} trip to ${dest} for ${guests} guest${guests === 1 ? '' : 's'} ${budgetLabel} per night with a day-by-day itinerary.`;
      navigate(`/ai-planner?prompt=${encodeURIComponent(prompt)}`);
    }, 1200);
  };
