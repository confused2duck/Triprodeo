...
import { createRazorpayBookingOrder, verifyRazorpayBookingPayment } from '@/services/api';
...
export default function BookingPage() {
  ...
  const fallbackProperty = useMemo(() => {
    ...
    return cmsProperty || properties.find((p) => p.id === id) || properties[0];
  }, [id]);
  const [property, setProperty] = useState<Property | typeof fallbackProperty>(fallbackProperty);
  ...
  useEffect(() => {
    ...
    fetchPropertyById(id)
      .then(setProperty)
      .catch(() => setProperty(fallbackProperty));
  }, [fallbackProperty, id]);
  ...
  const selectedRoom = 'roomTypes' in property
    ...
    : undefined;
  const fallbackRoom = 'roomTypes' in property ? property.roomTypes[0] : undefined;
  const activeRoom = selectedRoom ?? fallbackRoom;
  const roomCapacity = activeRoom?.capacity ?? property.maxGuests;
  const roomPrice = activeRoom?.pricePerNight ?? property.pricePerNight;
  const minimumRooms = Math.max(1, Math.ceil(guests / Math.max(1, roomCapacity)));
  const [availableRooms, setAvailableRooms] = useState<number | null>(null);
  const [availabilityLoading, setAvailabilityLoading] = useState(false);
  const [selectedRooms, setSelectedRooms] = useState(minimumRooms);
  const maxSelectableRooms = Math.max(minimumRooms, availableRooms ?? activeRoom?.totalRooms ?? minimumRooms);
  const roomsRequired = Math.max(minimumRooms, Math.min(selectedRooms, maxSelectableRooms));
  ...
  const canSubmit = datesValid && inventoryOk && !availabilityLoading && !isProcessing;
  ...
  useEffect(() => {
    ...
    apiFetch<{ availableRooms: number; bookedRooms: number; totalRooms: number; isAvailable: boolean }>(
      `/inventory/availability?roomId=${encodeURIComponent(roomIdForAvailability)}&propertyId=${encodeURIComponent(property.id)}&startDate=${encodeURIComponent(checkIn)}&endDate=${encodeURIComponent(checkOut)}`
    )
      .then((summary) => {
  ...
  }, [activeRoom?.id, checkIn, checkOut, property.id]);

  useEffect(() => {
    setSelectedRooms((current) => Math.max(minimumRooms, Math.min(current, maxSelectableRooms)));
  }, [activeRoom?.id, maxSelectableRooms, minimumRooms]);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const order = await createRazorpayBookingOrder({
        ...
        roomId: activeRoom?.id || roomIdParam || undefined,
        guestName,
        ...
        paymentPercent: isPartialPayment ? partialAmount : 100,
      });
      ...
      const checkout = new window.Razorpay({
        ...
        handler: async (response) => {
          try {
            const confirmed = await verifyRazorpayBookingPayment({
              bookingId: order.booking.id,
              razorpayOrderId: response.razorpay_order_id,
              razorpayPaymentId: response.razorpay_payment_id,
              razorpaySignature: response.razorpay_signature,
            });
            setBookingId(confirmed.id);
            setIsConfirmed(true);
            window.scrollTo({ top: 0, behavior: 'smooth' });
          } catch (error) {
            alert(error instanceof Error ? error.message : 'Unable to verify payment');
          } finally {
                    ...
                    <div className="rounded-xl border border-stone-200 bg-stone-50 p-4 space-y-3">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-2">
                          <i className="ri-hotel-bed-line text-stone-600 text-lg" />
                          <div>
                            <p className="text-sm font-semibold text-stone-900">
                              {activeRoom ? activeRoom.name : 'Standard Room'}
                            </p>
                            <p className="text-xs text-stone-500 mt-0.5">
                              Up to {roomCapacity} guest{roomCapacity === 1 ? '' : 's'} per room
                              {activeRoom?.pricePerNight ? ` · ₹${activeRoom.pricePerNight.toLocaleString('en-IN')}/night` : ''}
                            </p>
                          </div>
                        </div>
                        {/* Availability pill */}
                        {activeRoom && (
                          <div className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full shrink-0 ${
                            ...
                            <i className={availabilityLoading ? 'ri-loader-4-line animate-spin' : availableRooms === 0 ? 'ri-close-circle-line' : 'ri-checkbox-circle-line'} />
                            {availabilityLoading ? 'Checking...' : availableRooms === null ? 'Select dates' : availableRooms === 0 ? 'Sold out' : `${availableRooms} left`}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between border-t border-stone-200 pt-3">
                        <div>
                          <p className="text-xs font-medium text-stone-700">Number of Rooms</p>
                          <p className="text-xs text-stone-400 mt-0.5">
                            Min {minimumRooms} for {guests} guest{guests === 1 ? '' : 's'}
                            {maxSelectableRooms > minimumRooms ? `, max ${maxSelectableRooms}` : ''}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => setSelectedRooms((current) => Math.max(minimumRooms, current - 1))}
                            disabled={roomsRequired <= minimumRooms}
                            className="w-9 h-9 rounded-lg border border-stone-200 bg-white text-stone-700 disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed flex items-center justify-center"
                          >
                            <i className="ri-subtract-line" />
                          </button>
                          <span className="w-8 text-center font-bold text-stone-900 text-base">{roomsRequired}</span>
                          <button
                            type="button"
                            onClick={() => setSelectedRooms((current) => Math.min(maxSelectableRooms, current + 1))}
                            disabled={roomsRequired >= maxSelectableRooms}
                            className="w-9 h-9 rounded-lg border border-stone-200 bg-white text-stone-700 disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed flex items-center justify-center"
                          >
                      ...
                      {/* Not enough rooms warning */}
                      {availableRooms !== null && availableRooms > 0 && roomsRequired > availableRooms && (
                        <p className="text-xs text-red-500 flex items-center gap-1">
                          <i className="ri-error-warning-line" />
                          Only {availableRooms} room{availableRooms === 1 ? '' : 's'} available for these dates.
                        </p>
                      )}
                    </div>
              ...
              {step === 3 && (
                <div className="space-y-6">
                  ...
                  <div className="bg-white rounded-2xl border border-stone-200 p-6 md:p-8">
                    ...
                    {!datesValid ? (
                      <div className="mb-4 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm flex items-center gap-2">
                        <i className="ri-calendar-line" />
                        Pick valid check-in and check-out dates before paying.
                      </div>
                    ) : !inventoryOk ? (
                      <div className="mb-4 px-3 py-2.5 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm flex items-center gap-2">
                        <i className="ri-error-warning-line" />
                        {availableRooms === null
                          ? 'Unable to verify room inventory for these dates.'
                          : availableRooms === 0
                          ? 'SOLD OUT'
                    ...
                    <form onSubmit={handlePaymentSubmit} className="space-y-4">
                      <div className="rounded-xl border border-stone-200 bg-stone-50 p-4 text-sm text-stone-600 flex items-start gap-3">
                        <i className="ri-secure-payment-line text-stone-900 text-lg mt-0.5" />
                        <div>
                          <p className="font-semibold text-stone-900">Secure Razorpay Checkout</p>
                          <p className="mt-1">You can pay by card, UPI, net banking, or wallet in the Razorpay window.</p>
                        </div>
                      </div>
                      <div>
                  ...
                  <div className="p-4 border-b border-stone-100 space-y-3">
                    ...
                    {activeRoom && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-stone-600 truncate">{activeRoom.name}</span>
                        {availableRooms !== null && availableRooms > 0 && (
                          <span className="ml-auto text-xs text-emerald-600 whitespace-nowrap shrink-0">
                            {availableRooms} avail.
                          </span>
                        )}
                        {availableRooms === 0 && (
                          <span className="ml-auto text-xs text-red-500 font-semibold whitespace-nowrap shrink-0">Sold out</span>
                        )}
                      </div>
                    )}
                  </div>
