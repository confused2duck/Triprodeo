...
export const defaultPropertyTagOptions = ['Corporate Getaways', 'School Trips', 'Private Stays', 'Pet Friendly', 'Beach front'];

export const defaultPropertyTypeOptions = ['villa', 'resort', 'boutique', 'treehouse', 'beachfront', 'houseboat', 'lodge', 'farmstay'];

export const defaultProperties: CMSProperty[] = [
  {
    id: 'p1',
    ...
    reviews: [
      ...
      { id: 'r2', user: 'Verified Guest', avatar: 'https://readdy.ai/api/search-image?query=indian%20couple%20smiling%20selfie%20portrait%20happy%20travel%20vacation&width=60&height=60&seq=rv2&orientation=squarish', location: 'Bengaluru', date: 'February 2025', rating: 5, text: 'We booked this for our honeymoon and it exceeded every expectation. The BBQ setup on the cliff edge overlooking the sea was pure magic. This is luxury without pretension.' },
    ],
    roomTypes: [
...
export const defaultCMSData: CMSData = {
  ...
  propertyTagOptions: defaultPropertyTagOptions,
  ...
  about: {
    ...
    storyParagraph1: 'It was 2019, and the Triprodeo founding team was trying to plan a weekend getaway. After hours of browsing generic booking sites, they realized that truly special stays — the kind that create memories — were nearly impossible to find. That frustration became the seed of Triprodeo.',
    ...
    founders: [
      { name: 'Triprodeo Team', role: 'Founding Team', bio: 'Travel and hospitality operators building curated stays across India.', image: 'https://readdy.ai/api/search-image?query=professional%20indian%20businessman%20portrait%20confident%20smile%20modern%20office%20setting%20corporate%20headshot&width=200&height=200&seq=founder1&orientation=squarish' },
...
function withOptionDefaults(data: CMSData): CMSData {
  const propertyTags = (data.properties ?? []).flatMap((p) => p.tags ?? []);
  const propertyTypes = (data.properties ?? []).map((p) => p.type).filter(Boolean);
  return {
    ...data,
    propertyTagOptions: Array.from(new Set([...(data.propertyTagOptions ?? defaultPropertyTagOptions), ...propertyTags])),
    propertyTypeOptions: Array.from(new Set([...(data.propertyTypeOptions ?? defaultPropertyTypeOptions), ...propertyTypes])),
    trendingDestinations: data.trendingDestinations ?? defaultTrendingDestinations,
  };
}
...
export function loadCMSData(): CMSData {
  try {
    ...
    if (stored) {
      ...
      parsed.properties = dedupeById(parsed.properties);
      ...
      return withOptionDefaults(parsed);
    }
  } catch {
    // fallback to default
  }
...
export async function initCMSData(): Promise<void> {
  try {
    ...
    if (remote && typeof remote === 'object' && 'properties' in remote) {
      localStorage.setItem(CMS_STORAGE_KEY, JSON.stringify(withOptionDefaults(remote)));
    }
  } catch {
...
export async function fetchCMSData(): Promise<CMSData> {
  try {
    const remote = await apiFetch<CMSData | null>('/cms/public-content');
    if (remote && typeof remote === 'object' && 'properties' in remote) {
      const normalized = withOptionDefaults(remote);
      localStorage.setItem(CMS_STORAGE_KEY, JSON.stringify(normalized));
      return normalized;
    }
  } catch {
    // Backend unreachable or not initialized; use the local CMS cache/defaults.
  }
  return loadCMSData();
}
