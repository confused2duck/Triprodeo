...
import PressReleasesEditor from './components/PressReleasesEditor';
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case 'tags':
        return (
          <TagsEditor
            tags={cmsData.propertyTagOptions ?? []}
            onSave={(propertyTagOptions) => updateData({ propertyTagOptions })}
          />
        );
      case 'properties':
        return (
          <PropertiesEditor
            data={cmsData.properties}
            tagOptions={cmsData.propertyTagOptions ?? []}
            typeOptions={cmsData.propertyTypeOptions ?? []}
            onSave={(properties) => updateData({ properties })}
            onSaveOptions={(propertyTagOptions, propertyTypeOptions) => updateData({ propertyTagOptions, propertyTypeOptions })}
          />
        );
      ...
      case 'press': return <PressReleasesEditor data={cmsData.pressReleases ?? []} onSave={(pressReleases) => updateData({ pressReleases })} />;
