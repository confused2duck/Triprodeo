...
export default function BookingsManager() {
  const [allBookings, setAllBookings] = useState<HostBooking[]>([]);
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        const data = await apiFetch<{ bookings?: HostBooking[] }>('/bookings/admin/all', {
          headers: authHeader(),
        });
  ...
  const [enquiries, setEnquiries] = useState<DayOutingEnquiry[]>([]);
  ...
  const fetchDayOutingEnquiries = async () => {
    try {
      setEnquiriesLoading(true);
      setEnquiriesError(null);
      const data = await apiFetch<{ enquiries?: DayOutingEnquiry[] }>('/day-outing/enquiries?limit=100', {
        headers: authHeader(),
      });
      setEnquiries(data.enquiries || []);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to load day outing enquiries';
      setEnquiriesError(message);
      console.error('Failed to fetch day outing enquiries:', err);
    } finally {
  ...
  const applyDayBulk = async () => {
    if (!dayBulkAction || selectedDayIds.size === 0) return;
    try {
      await Promise.all(
        Array.from(selectedDayIds).map((id) =>
          apiFetch(`/day-outing/enquiries/${id}/status`, {
            method: 'PATCH',
            headers: authHeader(),
            body: JSON.stringify({ status: dayBulkAction }),
          })
        )
      );
      setSelectedDayIds(new Set());
      setDayBulkAction('');
      await fetchDayOutingEnquiries();
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to update selected day outing enquiries';
      setEnquiriesError(message);
    }
  };

  const updateDayStatus = async (id: string, status: DayOutingEnquiry['status']) => {
    try {
      const updated = await apiFetch<DayOutingEnquiry>(`/day-outing/enquiries/${id}/status`, {
        method: 'PATCH',
        headers: authHeader(),
      ...
      const message = err instanceof Error ? err.message : 'Failed to update day outing enquiry';
      setEnquiriesError(message);
    }
  };
  ...
  return (
    <div>
      ...
      {bookingsError && (
        <div className="flex items-center gap-3 px-4 py-3 bg-red-50 border border-red-200 text-red-700 rounded-xl mb-6 text-sm">
      ...
      {mainTab === 'dayouting' && (
        <>
          {enquiriesLoading && (
            <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 border border-blue-200 text-blue-700 rounded-xl mb-6 text-sm">
              <i className="ri-loader-4-line animate-spin" />
              Loading day outing enquiries from database...
            </div>
          )}
          {enquiriesError && (
            <div className="flex items-center gap-3 px-4 py-3 bg-red-50 border border-red-200 text-red-700 rounded-xl mb-6 text-sm">
              <i className="ri-alert-line" />
              Error loading day outing enquiries: {enquiriesError}
            </div>
          )}
