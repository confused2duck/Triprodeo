...
import { CMSExperience } from '@/pages/admin/types';

const toExperience = (experience: CMSExperience): Experience => {
  const [city = experience.location, state = ''] = experience.location.split(',').map((part) => part.trim());
  return {
    id: experience.id,
    ...
    subcategory: experience.category,
    image: experience.image,
    gallery: experience.image ? [experience.image] : [],
    description: `${experience.title} in ${experience.location}`,
    highlights: [],
    includes: [],
    excludes: [],
    groupSize: 'Open group',
    languages: ['English', 'Hindi'],
    cancellation: 'Cancellation policy as confirmed during booking',
    host: {
      name: 'Triprodeo',
      avatar: '',
      bio: 'Curated by Triprodeo',
      verified: true,
      experience: '',
    },
    availability: {
      dates: ['On request'],
      timeSlots: [experience.duration],
    },
    similarExperiences: [],
  };
};
...
export default function ExperiencesPage() {
  ...
  const [cmsExperiences, setCmsExperiences] = useState<Experience[]>(() => {
    const saved = loadCMSData().experiences;
    return saved.length ? saved.map(toExperience) : experiences;
  });

  useEffect(() => {
    fetchCMSData()
      .then((data) => {
        if (data.experiences.length > 0) setCmsExperiences(data.experiences.map(toExperience));
      })
      .catch(() => undefined);
  }, []);
  ...
  const cmsCategories = useMemo(() => {
    ...
    cmsExperiences.forEach((experience) => counts.set(experience.category, (counts.get(experience.category) ?? 0) + 1));
    return [
      { id: 'all', name: 'All', icon: 'ri-apps-line', count: cmsExperiences.length },
      ...Array.from(counts.entries()).map(([category, count]) => ({
        id: category,
        name: category,
        icon: experienceCategories.find((item) => item.id === category)?.icon ?? 'ri-compass-3-line',
        count,
      })),
    ];
  }, [cmsExperiences]);
  ...
  const filteredExperiences = useMemo(() => {
    ...
    return result;
  }, [activeCategory, activePriceRange, activeDuration, activeRating, sortBy, cmsExperiences]);
        ...
        {!showBundles ? (
          <>
            ...
            <p className="text-stone-500 text-sm mb-6">
              ...
              {activeCategory !== 'all' && ` in ${cmsCategories.find(c => c.id === activeCategory)?.name}`}
            </p>
