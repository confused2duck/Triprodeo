...
const AMENITY_PRESETS = [
  ...
  'Private Pool', 'Butler Service', 'Kitchenette', 'Extra Bed Available',
];
      ...
      <div>
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-6 space-y-5 max-w-2xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="flex items-center gap-2 self-end px-4 py-2.5 border border-stone-200 rounded-xl text-sm text-stone-700 cursor-pointer">
              <input
                type="checkbox"
                checked={form.amenities.includes(EXTRA_BED_AMENITY)}
                onChange={(e) => setForm({
                  ...form,
                  amenities: e.target.checked
                    ? Array.from(new Set([...form.amenities, EXTRA_BED_AMENITY]))
                    : form.amenities.filter((item) => item !== EXTRA_BED_AMENITY),
                })}
