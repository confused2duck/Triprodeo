...
import { HostProperty, CMSAddOn, CMSRoomType, CMSDayPackageOption } from '@/pages/admin/types';
import { defaultPropertyTagOptions, defaultPropertyTypeOptions, fetchCMSData, loadCMSData } from '@/pages/admin/cmsStore';
...
type FormSection = 'basic' | 'rooms' | 'addons' | 'daypackage';
...
const emptyDayPackageOption = (): CMSDayPackageOption => ({
...
const normalizeRoomTypes = (roomTypes: unknown): CMSRoomType[] => Array.isArray(roomTypes)
...
const normalizeSavedProperty = (property: any): HostProperty => ({
  ...property,
  status: String(property.status ?? 'pending').toLowerCase(),
  roomTypes: normalizeRoomTypes(property.roomTypes),
...
export default function HostPropertiesManager({ hostId, properties, onUpdate }: Props) {
  ...
  const [tagOptions, setTagOptions] = useState<string[]>(() => Array.from(new Set([...(loadCMSData().propertyTagOptions ?? []), ...defaultPropertyTagOptions])));
  const [typeOptions, setTypeOptions] = useState<string[]>(() => Array.from(new Set([...(loadCMSData().propertyTypeOptions ?? []), ...defaultPropertyTypeOptions])));
  ...
  useEffect(() => {
    fetchCMSData()
      .then((data) => {
        setTagOptions(Array.from(new Set([...(data.propertyTagOptions ?? []), ...defaultPropertyTagOptions])));
        setTypeOptions(Array.from(new Set([...(data.propertyTypeOptions ?? []), ...defaultPropertyTypeOptions])));
      })
      .catch(() => undefined);
  }, []);

  useEffect(() => {
    setTagOptions((current) => Array.from(new Set([...current, ...properties.flatMap((property) => property.tags ?? [])])));
    setTypeOptions((current) => Array.from(new Set([...current, ...properties.map((property) => property.type).filter(Boolean)])));
  }, [properties]);
  ...
  const openEdit = (p: HostProperty) => {
    ...
    setForm({
      ...
      roomTypes: normalizeRoomTypes(p.roomTypes),
  ...
  const handleDelete = (id: string) => {
    ...
    updateProperty(id, { status: 'inactive' }).finally(() => onUpdate(updated));
  };

  const handleSave = async () => {
    if (!form.name || !form.location || !form.pricePerNight) return;
    const normalizedRooms = form.roomTypes ?? [];
    const derivedMaxGuests = normalizedRooms.length > 0
      ? Math.max(1, normalizedRooms.reduce((total, room) => total + (Number(room.capacity) || 1) * (Number(room.totalRooms) || 1), 0))
      : form.maxGuests;

    if (view === 'edit' && editing) {
      const payload = {
        ...form,
        isExclusive: editing.isExclusive ?? false,
        maxGuests: derivedMaxGuests,
        roomTypes: normalizedRooms,
        amenities: form.amenities ?? [],
        hostId,
        housePolicies: form.housePolicies ?? [],
        dayPackage: form.dayPackage,
        addOns: form.addOns ?? [],
      };
      const savedProperty = await updateProperty(editing.id, payload);
      onUpdate(properties.map((p) => (p.id === editing.id ? normalizeSavedProperty(savedProperty) : p)));
    } else {
      const savedProperty = await createProperty({
        ...
        maxGuests: derivedMaxGuests,
        roomTypes: normalizedRooms,
        status: form.status === 'active' ? 'approved' : form.status === 'inactive' ? 'inactive' : 'pending',
        verified: false,
        isExclusive: false,
        housePolicies: form.housePolicies ?? [],
        dayPackage: form.dayPackage ?? undefined,
      ...
      onUpdate([...properties, normalizeSavedProperty(savedProperty)]);
    }
    setSaved(true);
  ...
  const dpPackages = form.dayPackage?.packages ?? [];
        ...
        <div className="flex gap-1 mb-5 bg-stone-100 p-1 rounded-xl w-fit">
          {([
            ...
            >
              ...
              {s.id === 'rooms' && (form.roomTypes?.length ?? 0) > 0 && (
                <span className="ml-1 px-1.5 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">{form.roomTypes?.length}</span>
              )}
        ...
        {section === 'rooms' && (
          <div className="bg-white rounded-xl border border-stone-200 p-6 max-w-3xl">
            <div className="flex items-start justify-between gap-4 mb-5">
              <div>
                <h3 className="text-sm font-semibold text-stone-700">Room Types</h3>
                <p className="text-stone-400 text-xs mt-1">Add each room category with price, room count, guest capacity, and extra bed availability. Customer booking capacity uses these room limits.</p>
              </div>
              <button
                type="button"
                onClick={() => setForm({ ...form, roomTypes: [...(form.roomTypes ?? []), emptyRoom()] })}
                className="px-4 py-2 bg-stone-900 text-white rounded-lg text-sm font-semibold cursor-pointer whitespace-nowrap"
              >
                <i className="ri-add-line mr-1" /> Add Room Type
              </button>
            </div>

            {(form.roomTypes ?? []).length === 0 ? (
              <div className="py-10 text-center border-2 border-dashed border-stone-200 rounded-2xl bg-stone-50">
                <i className="ri-hotel-bed-line text-4xl text-stone-300 block mb-3" />
                <p className="text-sm font-semibold text-stone-700 mb-1">No room types added</p>
                <p className="text-xs text-stone-400 mb-4">Add at least one room type to enable room-based bookings.</p>
                <button type="button" onClick={() => setForm({ ...form, roomTypes: [emptyRoom()] })} className="px-5 py-2 bg-stone-900 text-white rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap">Add First Room</button>
              </div>
            ) : (
              <div className="space-y-5">
                {(form.roomTypes ?? []).map((room, index) => {
                  const updateRoom = (patch: Partial<CMSRoomType>) => {
                    const rooms = [...(form.roomTypes ?? [])];
                    rooms[index] = { ...room, ...patch };
                    const derivedMaxGuests = Math.max(1, rooms.reduce((total, r) => total + (Number(r.capacity) || 1) * (Number(r.totalRooms) || 1), 0));
                    setForm({ ...form, roomTypes: rooms, maxGuests: derivedMaxGuests });
                  };
                  const extraBed = (room.amenities ?? []).includes('Extra Bed Available');
                  return (
                    <div key={room.id} className="border border-stone-200 rounded-2xl p-4 space-y-4">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-semibold text-stone-800">Room Type {index + 1}</p>
                        <button
                          type="button"
                          onClick={() => {
                            const rooms = (form.roomTypes ?? []).filter((_, i) => i !== index);
                            setForm({ ...form, roomTypes: rooms, maxGuests: Math.max(1, rooms.reduce((total, r) => total + (Number(r.capacity) || 1) * (Number(r.totalRooms) || 1), 0)) });
                          }}
                          className="text-red-500 text-sm cursor-pointer whitespace-nowrap"
                        >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-medium text-stone-500 mb-1.5">Room Name</label>
                          <input value={room.name} onChange={(e) => updateRoom({ name: e.target.value })} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400" placeholder="Deluxe Room" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-stone-500 mb-1.5">Price Per Night (₹)</label>
                          <input type="number" value={room.pricePerNight || ''} onChange={(e) => updateRoom({ pricePerNight: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-stone-500 mb-1.5">Capacity Per Room</label>
                          <input type="number" min={1} value={room.capacity || 1} onChange={(e) => updateRoom({ capacity: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-stone-500 mb-1.5">Total Rooms</label>
                          <input type="number" min={1} value={room.totalRooms || 1} onChange={(e) => updateRoom({ totalRooms: Number(e.target.value) })} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-stone-500 mb-1.5">Bed Type</label>
                          <input value={room.bedType ?? ''} onChange={(e) => updateRoom({ bedType: e.target.value })} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400" placeholder="King Bed" />
                        </div>
                        <label className="flex items-center gap-3 px-3 py-2 border border-stone-200 rounded-lg bg-stone-50 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={extraBed}
                            onChange={(e) => updateRoom({ amenities: e.target.checked ? Array.from(new Set([...(room.amenities ?? []), 'Extra Bed Available'])) : (room.amenities ?? []).filter((item) => item !== 'Extra Bed Available') })}
                          />
                          <span className="text-sm text-stone-700">Provision of extra bed</span>
                        </label>
                        <div className="sm:col-span-2">
                          <ImageUploader
                            images={(room.photos ?? []).filter(Boolean)}
                            onChange={(imgs) => updateRoom({ photos: imgs })}
                            label="Room Photos"
                            multiple={true}
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <textarea value={room.description ?? ''} onChange={(e) => updateRoom({ description: e.target.value })} rows={2} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-stone-400 resize-none" placeholder="Short room description..." />
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setView('list')} className="px-6 py-2.5 border border-stone-200 text-stone-700 rounded-xl text-sm font-medium hover:bg-stone-50 cursor-pointer whitespace-nowrap">Cancel</button>
              <button onClick={handleSave} className="flex-1 py-2.5 bg-stone-900 text-white rounded-xl text-sm font-semibold hover:bg-stone-800 cursor-pointer whitespace-nowrap">Save Changes</button>
            </div>
          </div>
        )}
            ...
            {!dpEnabled ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between rounded-xl border border-amber-100 bg-amber-50 p-3">
                  <div>
                    <p className="text-sm font-semibold text-stone-800">Package Variants</p>
                    <p className="text-xs text-stone-500">Add multiple day packages such as half-day, full-day, school trip, or corporate outing.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => updateDp({ packages: [...dpPackages, emptyDayPackageOption()] })}
                    className="px-3 py-2 bg-white border border-amber-200 text-stone-700 rounded-lg text-xs font-semibold cursor-pointer whitespace-nowrap"
                  >
                    <i className="ri-add-line mr-1" /> Add Package
                  </button>
                </div>
                {dpPackages.length > 0 && (
                  <div className="space-y-3">
                    {dpPackages.map((pkg, index) => {
                      const updatePkg = (patch: Partial<CMSDayPackageOption>) => {
                        const packages = [...dpPackages];
                        packages[index] = { ...pkg, ...patch };
                        updateDp({ packages });
                      };
                      return (
                        <div key={pkg.id} className="border border-stone-200 rounded-xl p-3 space-y-3">
                          <div className="flex items-center justify-between">
                            <p className="text-xs font-semibold text-stone-700">Variant {index + 1}</p>
                            <button type="button" onClick={() => updateDp({ packages: dpPackages.filter((_, i) => i !== index) })} className="text-xs text-red-500 cursor-pointer">Remove</button>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <input value={pkg.title} onChange={(e) => updatePkg({ title: e.target.value })} className="px-3 py-2 border border-stone-200 rounded-lg text-sm" placeholder="Package title" />
                            <input value={pkg.timing ?? ''} onChange={(e) => updatePkg({ timing: e.target.value })} className="px-3 py-2 border border-stone-200 rounded-lg text-sm" placeholder="Timing" />
                            <input type="number" value={pkg.pricePerPerson || ''} onChange={(e) => updatePkg({ pricePerPerson: Number(e.target.value) })} className="px-3 py-2 border border-stone-200 rounded-lg text-sm" placeholder="Price per person" />
                            <input type="number" value={pkg.maxGuests || ''} onChange={(e) => updatePkg({ maxGuests: e.target.value ? Number(e.target.value) : undefined })} className="px-3 py-2 border border-stone-200 rounded-lg text-sm" placeholder="Max guests" />
                            <textarea value={pkg.description ?? ''} onChange={(e) => updatePkg({ description: e.target.value })} rows={2} className="sm:col-span-2 px-3 py-2 border border-stone-200 rounded-lg text-sm resize-none" placeholder="Description" />
                          </div>
                        </div>
                      );
                    })}
