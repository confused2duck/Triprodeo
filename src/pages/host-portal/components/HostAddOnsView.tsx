...
export default function HostAddOnsView({ hostId, properties, onUpdate }: Props) {
  ...
  const persist = useCallback((updatedAddOns: CMSAddOn[]) => {
    ...
    onUpdate(properties.map((p) => p.id === updated.id ? updated : p));
    updateProperty(updated.id, { addOns: updatedAddOns })
      .then((savedProperty: any) => {
        const normalized: HostProperty = {
          ...savedProperty,
          status: String(savedProperty.status ?? updated.status).toLowerCase(),
          roomTypes: Array.isArray(savedProperty.roomTypes)
            ? savedProperty.roomTypes.map((room: any) => ({
                ...room,
                photos: Array.isArray(room.photos) ? room.photos : Array.isArray(room.images) ? room.images : [],
                amenities: Array.isArray(room.amenities) ? room.amenities : [],
              }))
            : updated.roomTypes,
          addOns: Array.isArray(savedProperty.addOns)
            ? savedProperty.addOns.map((addOn: any) => ({
                ...addOn,
                image: addOn.image ?? '',
        ...
        onUpdate(properties.map((p) => p.id === updated.id ? normalized : p));
        setSaved(true);
  ...
  }, [selectedProp, hostId, onUpdate, properties]);
