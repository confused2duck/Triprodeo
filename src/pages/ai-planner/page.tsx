...
export default function AIPlannerPage() {
  ...
  // Pre-seed state from URL ?destination= so the RAG engine strictly filters
  // from the very first turn instead of showing all-location results.
  const [plannerState, setPlannerState] = useState<QueryState | undefined>(() => {
  ...
  useEffect(() => {
    ...
    if (!prompt || initialPromptSent || corpus.length === 0) return;
    setInitialPromptSent(true);
    handleSendMessage(prompt);
  }, [corpus.length, handleSendMessage, initialPromptSent, searchParams]);
