...
const EXTRA_DESTINATIONS: string[] = [];
...
export function retrieveProperties(
  ...
  if (placeQuery) {
    ...
    // Strict: if user named a destination, only show properties from that destination.
    // Return empty rather than falling back to all properties.
    candidates = matched;
...
const NEARBY_PLACES: Record<string, string[]> = {
  goa: ['Candolim Beach', 'Fort Aguada', 'Sinquerim Beach', 'Fontainhas', 'Chapora Fort'],
  candolim: ['Candolim Beach', 'Fort Aguada', 'Sinquerim Beach', 'Calangute Market'],
  manali: ['Solang Valley', 'Hadimba Temple', 'Old Manali', 'Vashisht Hot Springs'],
  'himachal pradesh': ['Solang Valley', 'Hadimba Temple', 'Beas River viewpoints', 'Old Manali'],
  kerala: ['Alleppey backwaters', 'Marari Beach', 'local village canals', 'sunset canoe route'],
  alleppey: ['Alleppey backwaters', 'Marari Beach', 'local village canals', 'Alappuzha lighthouse'],
  rajasthan: ['local heritage market', 'fort viewpoint', 'traditional dinner experience', 'craft bazaar'],
  udaipur: ['Lake Pichola', 'City Palace', 'Jag Mandir', 'Saheliyon Ki Bari'],
  'sawai madhopur': ['Ranthambore National Park', 'Ranthambore Fort', 'Surwal Lake', 'local craft market'],
};

function getNearbyPlaces(property: Property): string[] {
  const keys = [property.city, property.state, property.location]
    .filter(Boolean)
    .map((value) => value.toLowerCase());
  for (const key of keys) {
    const direct = NEARBY_PLACES[key];
    if (direct) return direct;
    const fuzzy = Object.entries(NEARBY_PLACES).find(([place]) => key.includes(place));
    if (fuzzy) return fuzzy[1];
  }
  return [`Explore ${property.location}`, `Local sightseeing near ${property.city}`, 'nearby cafe or viewpoint'];
}
...
function buildItinerary(state: QueryState, top: Property): ChatMessage {
  ...
  const tags = top.tags ?? [];
  const nearbyPlaces = getNearbyPlaces(top);
  const nearby = [
    `Explore ${top.location}`,
    ...nearbyPlaces.map((place) => `Visit ${place}`),
...
export function runPlannerTurn(
  ...
  if (state.wantItinerary && !state.city && !state.state) {
