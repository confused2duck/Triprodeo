...
export default function DayPackageEnquiryModal({ propertyName, pricePerPerson, timing, propertyId, maxGuests, onClose }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    if (!propertyId) {
      setError('Unable to identify this property. Please reopen the page and try again.');
      return;
    }
    setLoading(true);
    try {
      await apiFetch('/day-outing/enquiries', {
        method: 'POST',
        headers: authHeader(),
        body: JSON.stringify({
          propertyId,
          ...
          packageTitle: timing || selectedSlot.label,
          guests: groupSize,
          pricePerPerson,
      ...
      >
        ...
        {step === 'confirm' && (
          <form data-readdy-form onSubmit={handleSubmit} className="px-6 py-5 max-h-[70vh] overflow-y-auto">
            ...
            {error && (
              <p className="mt-3 text-center text-red-600 text-xs font-medium">{error}</p>
            )}
