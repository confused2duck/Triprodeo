...
export default function DayOutingPage() {
  ...
  const [allDayOutingProperties, setAllDayOutingProperties] = useState<CompareProperty[]>([]);

  useEffect(() => {
    fetchProperties({ status: 'approved', limit: 500 })
      .then((data) => {
        setAllDayOutingProperties(
          data.properties
            .filter((p) => p.dayPackage?.enabled)
            .map((p) => ({
              id: p.id,
              name: p.name,
              location: p.location,
              image: p.dayPackage?.image || p.images[0],
              price: p.dayPackage?.pricePerPerson || 0,
      ...
      .catch(() => setAllDayOutingProperties([]));
  }, []);
